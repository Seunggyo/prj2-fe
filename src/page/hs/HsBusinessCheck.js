...
export function HsBusinessCheck() {
    ...
    useEffect(() => {
        ...
        axios.get("/api/hospital/reservation/business/" + id)
            .then((e) => updateList(e.data));
        axios.get("/api/hospital/reservation/business/check/" + id)
    ...
    function handleOkClick(e, id) {
        ...
        axios.put(`/api/hospital/reservation/ok/${id}`)
            .then(() => {
                toast({
            ...
            {list && list.length !== 0 ? (
                list.map(l => (
                    ...
                    <Card key={l.id}>
                        ...
                        <CardFooter>
                            <Button id={l.id} colorScheme={"green"} onClick={(e) => handleOkClick(e, l.id)}>예약
                                확인</Button>
                ...
                checkList.map(c => (
                    <Card key={c.id}>
                        ...
                        <CardBody>
                            ...
                            <Box>예약 날짜 : {new Date(c.reservationDate).toLocaleDateString()}</Box>
                            <Box>예약 시간
                                : {c.reservationHour < 13 ? "오전 " + c.reservationHour : "오후 " + (c.reservationHour - 12)} : {c.reservationMin === 0 ? "0" + c.reservationMin : c.reservationMin}</Box>
                            <Box>특이사항 : {c.comment}</Box>
                        </CardBody>
                        <CardFooter>
                            <Button id={c.id} colorScheme={"red"} onClick={e => {
                                e.stopPropagation();
                                onOpen();
                                setPosition(c.id)
                            }}>
