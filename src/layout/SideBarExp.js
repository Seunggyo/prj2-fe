...
export function SideBarExp() {
  const navigate = useNavigate();
  const { isOpen, onClose } = useDisclosure();

  return (
    <div className="relative bg-gray-50 dark:bg-slate-900 h-screen pattern">
      <nav className="z-20 flex-shrink-0 flex-grow-0 justify-around gap-4 border-t border-gray-200 bg-white/50 p-2.5 shadow-lg backdrop-blur-lg dark:border-slate-600/60 dark:bg-slate-800/50 fixed top-1/4 -translate-y-2/4 left-0 min-h-[auto] min-w-[64px] flex-col rounded-lg border">
        <a
          href="/"
      ...
      >
        ...
        {/* 여기에 장소 정보를 표시하는 내용을 추가할 수 있습니다. */}
      </Box>
    </div>
  );
}
